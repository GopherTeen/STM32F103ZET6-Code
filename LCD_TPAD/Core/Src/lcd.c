...
void lcd_scan_dir(uint8_t dir) {
    ...
    switch (dir) {
        case L2R_U2D: /* 从左到右,从上到下 */ regval |= (0 << 7) | (0 << 6) | (0 << 5); break;

        case L2R_D2U: /* 从左到右,从下到上 */ regval |= (1 << 7) | (0 << 6) | (0 << 5); break;

        case R2L_U2D: /* 从右到左,从上到下 */ regval |= (0 << 7) | (1 << 6) | (0 << 5); break;

        case R2L_D2U: /* 从右到左,从下到上 */ regval |= (1 << 7) | (1 << 6) | (0 << 5); break;

        case U2D_L2R: /* 从上到下,从左到右 */ regval |= (0 << 7) | (0 << 6) | (1 << 5); break;

        case U2D_R2L: /* 从上到下,从右到左 */ regval |= (0 << 7) | (1 << 6) | (1 << 5); break;

        case D2U_L2R: /* 从下到上,从左到右 */ regval |= (1 << 7) | (0 << 6) | (1 << 5); break;

        case D2U_R2L: /* 从下到上,从右到左 */ regval |= (1 << 7) | (1 << 6) | (1 << 5); break;
    }
...
void lcd_show_char(uint16_t x, uint16_t y, char chr, uint8_t size, uint8_t mode, uint16_t color) {
    ...
    for (int t = 0; t < csize; t++) {
        tmp = pfont[t]; /* 获取字符的点阵数据 */

        for (t1 = 0; t1 < 8; t1++) { // 一个字节 8 个点
            if (tmp & 0x80) { // 有效点,需要显示
                lcd_draw_point(x, y, color); /* 画点出来,要显示这个点 */
            } else if (mode == 0) { // 无效点,不显示
                lcd_draw_point(x, y, g_back_color); /* 画背景色,相当于这个点不显示(注意背景色由全局变量控制) */
            }

            tmp <<= 1; /* 移位, 以便获取下一个位的状态 */
            y++;
            if (y >= lcddev.height) { // y 坐标超区域了
                return;
            }
            if ((y - y0) == size) { // 显示完一列
                y = y0;
                x++;
                if (x >= lcddev.width) { // x 坐标超区域了
                    return;
                }
                break;
            }
...
void lcd_show_num(uint16_t x, uint16_t y, uint32_t num, uint8_t len, uint8_t size, uint16_t color) {
    uint8_t tmp;
    uint8_t enshow = 0;
    for (int t = 0; t < len; t++) { // 按总显示位数循环
        tmp = (num / lcd_pow(10, len - t - 1)) % 10; // 获取对应位的数字
        ...
        lcd_show_char(x + (size / 2) * t, y, tmp + '0', size, 0, color); /* 显示字符 */
    }
}
...
void lcd_show_xnum(uint16_t x, uint16_t y, uint32_t num, uint8_t len, uint8_t size, uint8_t mode, uint16_t color) {
    ...
    for (int t = 0; t < len; t++) { // 按总显示位数循环
        ...
        lcd_show_char(x + (size / 2) * t, y, tmp + '0', size, mode & 0X01, color);
    }
}

...
#define LCD_BL(x)                                                                                                                            \
    do {                                                                                                                                     \
        x ? HAL_GPIO_WritePin(LCD_BL_GPIO_Port, LCD_BL_Pin, GPIO_PIN_SET) : HAL_GPIO_WritePin(LCD_BL_GPIO_Port, LCD_BL_Pin, GPIO_PIN_RESET); \
    } while (0)
...
/* LCD_BASE的详细解算方法:
 * 我们一般使用FSMC的块1(BANK1)来驱动TFTLCD液晶屏(MCU屏),
 * 块1地址范围总大小为256MB,均分成4块: 存储块1(FSMC_NE1)地址范围: 0X6000 0000 ~
 * 0X63FF FFFF 存储块2(FSMC_NE2)地址范围: 0X6400 0000 ~ 0X67FF FFFF
 ...
 * 战舰F103开发板使用FSMC_NE4连接LCD_CS, FSMC_A10连接LCD_RS
 * ,16位数据线,计算方法如下: 首先FSMC_NE4的基地址为: 0X6C00 0000;
 * NEx的基址为(x=1/2/3/4): 0X6000 0000 + (0X400 0000 * (x - 1))
 * FSMC_A10对应地址值: 2^10 * 2 = 0X800;    FSMC_Ay对应的地址为(y = 0 ~ 25): 2^y
 * * 2
 *
 ...
 *          等效于(使用移位操作)
 *          LCD_BASE = (0X6000 0000 + (0X400 0000 * (x - 1))) | ((1 << y) * 2
 * -2)
 */
